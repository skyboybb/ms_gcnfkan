...
import numpy as np
class train_TDmlpORKanORmc_cnn_utils(object):
    ...
    def setup(self):
        ...
        self.datasets_1['train'],self.datasets_1['val'],self.label['train'],self.label['val'] = Dataset(args.sample_length,args.data_dir, args.Input_type, args.task).data_preprare()
        self.N = self.datasets_1['train'].size(0)

        # Define the model
        sample_length=self.datasets_1['train'].size(1)

        #self.model = getattr(models, args.model_name)(feture=sample_length,out_channel=Dataset.num_classes) #其他模型使用这段代码
        self.model = getattr(models, args.model_name)(out_channel=Dataset.num_classes)  # MC_CNN使用这段代码
        #self.model = getattr(models, args.model_name)(num_blocks=[2,2,2,2],out_channel=Dataset.num_classes)  # RESNET使用这段代码
    ...
    def train(self):
        ...
        def compute_accuracy(model,feature_1,labels):
            ...
            for i in range(total_batch):
                ...
                logits, probas = model(features_1)
        ...
        for epoch in range(num_epochs):
            ...
            with torch.set_grad_enabled(False):  # save memory during inference
                trl, trac, _ = compute_accuracy(model, self.datasets_1['train'],self.label['train'])
                val, vaac, _ = compute_accuracy(model, self.datasets_1['val'],self.label['val'])
        ...
        torch.save(model, 'trained_model/MLPorKAN.pth')  # 保存整个网络参数
        # In[]
        # loss曲线
        ...
        model = torch.load('trained_model/MLPorKAN.pth')
        inputs_1 = self.datasets_1['val'].to(self.device)
        model.to(self.device)
        model.eval()
        logits, probas = model(inputs_1)
        ...
        _, teac_valid, _ = compute_accuracy(model, self.datasets_1['val'], self.label['val'])
        print('The correct rate of verification set is：', teac_valid.item(), '%')
