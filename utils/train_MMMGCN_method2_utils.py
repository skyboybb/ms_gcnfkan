...
class train_utils(object):
    ...
    def setup(self):
        ...
        self.datasets_1['train'],self.datasets_1['val'] = Dataset(args.sample_length,args.data_dir, args.Input_type, args.task).data_preprare()


        self.dataloaders_1 = {x: DataLoader(self.datasets_1[x], batch_size=args.batch_size, shuffle=False,num_workers=args.num_workers,
                                                           pin_memory=(True if self.device == 'cuda' else False))for x in ['train', 'val']}



        if args.task == 'Node':
            self.model = getattr(models, args.model_name)(feature=170,out_channel=Dataset.num_classes) ##图卷积使用这段代码
    ...
    def train(self):
        ...
        df.to_csv(r"D:\pycharm\pythonProject\ms_data\wpdCNN_3\GCNCU+KAN.csv", index=False)
        for epoch in range(self.start_epoch, args.max_epoch):
            ...
            for phase in ['train', 'val']:
                ...
                if phase == 'train':
                    ...
                    self.model.eval()
                sample_num = 0
                for data_1 in self.dataloaders_1[phase]:
                ...
                if phase == 'train':
                    ...
                    data.to_csv(r'D:\pycharm\pythonProject\ms_data\wpdCNN_3\GCNCU+KAN.csv', mode='a', header=False,
        ...
        model.eval()
        all_graphs=[]
        for batch in self.dataloaders_1['val']:
            # 将每个 Batch 对象中的图数据对象提取出来
            graphs = batch.to_data_list()
            all_graphs.extend(graphs)
        # 将所有的图数据对象合并成一个大的 Batch 对象
        input_x = Batch.from_data_list(all_graphs)
        input_x.to(self.device)
        input_y = input_x.y

        logits = model(input_x)
        pred = logits.argmax(dim=1)
        labels = input_y.detach().numpy()
        pre = pred.detach().numpy()
        C = confusion_matrix(labels, pre)

        # 混淆矩阵
        plot_confusion_matrix(C, ['0', '1', '2', '3', '4', '5', '6', '7','8'])
