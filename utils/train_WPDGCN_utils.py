...
class train_utils(object):
    ...
    def setup(self):
        ...
        self.datasets_1['train'], self.datasets_2['train'],self.datasets_3['train'],self.datasets_4['train'],\
        self.datasets_5['train'],self.datasets_6['train'], self.datasets_7['train'],self.datasets_8['train'],\
        self.datasets_1['val'], self.datasets_2['val'],self.datasets_3['val'], self.datasets_4['val'],\
        self.datasets_5['val'], self.datasets_6['val'],self.datasets_7['val'],  self.datasets_8['val'],\
            = Dataset(args.sample_length,args.data_dir, args.Input_type, args.task).data_preprare()
        ...
        self.dataloaders_3 = {x: DataLoader(self.datasets_3[x], batch_size=args.batch_size,
                                                           ...
                                                           num_workers=args.num_workers,
                                                           pin_memory=(True if self.device == 'cuda' else False))for x in ['train', 'val']}
        self.dataloaders_4 = {x: DataLoader(self.datasets_4[x], batch_size=args.batch_size,
                                                           shuffle=(False if x == 'train' else False),
                                                           num_workers=args.num_workers,
                                                           pin_memory=(True if self.device == 'cuda' else False))for x in ['train', 'val']}
        self.dataloaders_5 = {x: DataLoader(self.datasets_5[x], batch_size=args.batch_size,
                                                           shuffle=(False if x == 'train' else False),
                                                           num_workers=args.num_workers,
                                                           pin_memory=(True if self.device == 'cuda' else False))for x in ['train', 'val']}
        self.dataloaders_6 = {x: DataLoader(self.datasets_6[x], batch_size=args.batch_size,
                                                           shuffle=(False if x == 'train' else False),
                                                           num_workers=args.num_workers,
                                                           pin_memory=(True if self.device == 'cuda' else False))for x in ['train', 'val']}
        self.dataloaders_7 = {x: DataLoader(self.datasets_7[x], batch_size=args.batch_size,
                                                           shuffle=(False if x == 'train' else False),
                                                           num_workers=args.num_workers,
                                                           pin_memory=(True if self.device == 'cuda' else False))for x in ['train', 'val']}
        self.dataloaders_8 = {x: DataLoader(self.datasets_8[x], batch_size=args.batch_size,
                                                           shuffle=(False if x == 'train' else False),
                                                           num_workers=args.num_workers,
                                                           pin_memory=(True if self.device == 'cuda' else False))for x in ['train', 'val']}
        # Define the model
        ...
        if args.task == 'Node':
            self.model = getattr(models, args.model_name)(feature=feature,out_channel=Dataset.num_classes) ##图卷积使用这段代码
    ...
    def train(self):
        ...
        df.to_csv(r"D:\pycharm\pythonProject\ms_data\wpdCNN_3\WPD_CNN_3.csv", index=False)
        for epoch in range(self.start_epoch, args.max_epoch):
            ...
            for phase in ['train', 'val']:
                ...
                if phase == 'train':
                    ...
                    self.model.eval()
                sample_num = 0
                for data_1,data_2,data_3,data_4,data_5,data_6,data_7,data_8, in zip(self.dataloaders_1[phase],self.dataloaders_2[phase],\
                        self.dataloaders_3[phase],self.dataloaders_4[phase],self.dataloaders_5[phase],self.dataloaders_6[phase],\
                        self.dataloaders_7[phase],self.dataloaders_8[phase]):
                    inputs_1 = data_1.to(self.device)
                    inputs_2 = data_2.to(self.device)
                    inputs_3 = data_3.to(self.device)
                    inputs_4 = data_4.to(self.device)
                    inputs_5 = data_5.to(self.device)
                    inputs_6 = data_6.to(self.device)
                    inputs_7 = data_7.to(self.device)
                    inputs_8 = data_8.to(self.device)
                    labels = inputs_1.y
                    if args.task == 'Node':
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        # forward
                        if  args.task == 'Node':
                            logits = self.model(inputs_1,inputs_2,inputs_3,inputs_4,inputs_5,inputs_6,inputs_7,inputs_8)
                            #logits = self.model(inputs_1, inputs_2, inputs_3, inputs_4)
                            #logits = self.model(inputs_1, inputs_2)
                        else:
                            print("There is no such task!!")
                ...
                if phase == 'train':
                    ...
                    data.to_csv(r'D:\pycharm\pythonProject\ms_data\wpdCNN_3\WPD_CNN_3.csv', mode='a', header=False,
                                index=False)  # mode设为a,就可以向csv文件追加数据

        torch.save(self.model, 'trained_model/WPD_GCNCWRU.pth')  # 保存整个网络参数
        ...
        for data_1, data_2, data_3, data_4, data_5, data_6, data_7, data_8, in zip(self.dataloaders_1[phase],
                                                                                   self.dataloaders_2[phase], \
                                                                                   self.dataloaders_3[phase],
                                                                                   self.dataloaders_4[phase],
                                                                                   self.dataloaders_5[phase],
                                                                                   self.dataloaders_6[phase], \
                                                                                   self.dataloaders_7[phase],
                                                                                   self.dataloaders_8[phase]):
        ...
        model = torch.load('trained_model/WPD_GCNCWRU.pth')
        model.to(self.device)
        model.eval()
        logits = model(inputs_1, inputs_2, inputs_3, inputs_4, inputs_5, inputs_6, inputs_7, inputs_8)
        pred = logits.argmax(dim=1)
