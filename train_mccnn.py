...
import logging
from utils.train_TDmlpORKanORmc_cnn_utils import train_TDmlpORKanORmc_cnn_utils
args = None

def parse_args():
    parser = argparse.ArgumentParser(description='Train')
    # basic parameters
    parser.add_argument('--model_name', type=str, default='FKAN', help='the name of the model')#选择models文件_init_中的模型
    parser.add_argument('--sample_length', type=int, default=1024, help='batchsize of the training process')
    parser.add_argument('--data_name', type=str, default='GearTDorFFT_sets', help='the name of the data')#


    parser.add_argument('--Input_type', choices=['TD','FFT'],type=str, default='FFT', help='the input type decides the length of input')

    parser.add_argument('--data_dir', type=str, default= "./data/Gear", help='the directory of the data')
    ...
    # Define the tasks
    parser.add_argument('--task', choices=['Node'], type=str,
...
if __name__ == '__main__':
    ...
    trainer = train_TDmlpORKanORmc_cnn_utils(args, save_dir)
    trainer.setup()
    trainer.train()
